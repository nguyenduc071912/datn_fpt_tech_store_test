<template>
  <div class="pm">
    ...
    <!-- Tab bar -->
    <div class="pm-tabbar">
      <button v-for="t in tabs" :key="t.key" class="pm-tab"
    ...
    <div v-show="activeTab === 'set'" class="pm-pane">
      <div class="pm-grid2">
          ...
          <div class="pm-fields">
            <div class="pm-row2">
              <div class="pm-field">
                ...
                <input v-model.number="setForm.variantId" class="pm-in" placeholder="5"
                  type="number" @blur="loadCurrentPricePreview" />
              </div>
              <div class="pm-field">
            ...
            <div v-if="currentPricePreview" class="pm-alert pm-alert-info mb2">
              <span class="pm-rk">Giá đang active:</span>
              <strong class="ml1">{{ fmtMoney(currentPricePreview.price) }}</strong>
              <span class="pm-dim ml2">Nhập: {{ currentPricePreview.costPrice ? fmtMoney(currentPricePreview.costPrice) : "—" }}</span>
              <span v-if="currentPricePreview.belowCost" class="clr-red ml2">⚠️ Đang dưới giá nhập</span>
            </div>

            <div class="pm-row2">
              <div class="pm-field">
                <label class="pm-lbl">Giá bán</label>
                <input v-model.number="setForm.price" class="pm-in pm-in-money"
                  placeholder="42900000" type="number" />
                <span v-if="setForm.price" class="pm-in-hint">{{ fmtMoney(setForm.price) }}</span>
                <!-- Cảnh báo realtime -->
                <span v-if="setForm.price && setForm.costPrice && setForm.price < setForm.costPrice"
                  class="pm-in-hint clr-red">⚠️ Giá bán thấp hơn giá nhập!</span>
              </div>
              <div class="pm-field">
                <label class="pm-lbl">Giá nhập</label>
                <input v-model.number="setForm.costPrice" class="pm-in pm-in-money"
                  placeholder="36000000" type="number" />
                <span v-if="setForm.costPrice" class="pm-in-hint">{{ fmtMoney(setForm.costPrice) }}</span>
              </div>
            </div>
            <div class="pm-field">
              <label class="pm-lbl">Lý do</label>
              <input v-model="setForm.reason" class="pm-in"
                placeholder="MANUAL / PROMO_ENDED / PRICE_UPDATE" />
            </div>
          </div>
          <div class="pm-foot">
            <button class="pm-btn pm-btn-primary" :class="{ loading: setLoading }"
              @click="setPrice" :disabled="setLoading">
        ...
        <div class="pm-card">
          ...
          <button class="pm-btn pm-btn-outline" :class="{ loading: effLoading }"
            @click="loadEffective" :disabled="effLoading">
            <span v-if="!effLoading">⚡ Tải giá hiệu lực</span><span v-else class="pm-spin"></span>
          </button>
          <div v-if="effective" class="pm-result mt-3">
            <div class="pm-rrow"><span class="pm-rk">SKU</span>
              <span class="pm-rv mono">{{ effective.sku ?? "—" }}</span></div>
            <div class="pm-rrow"><span class="pm-rk">Base price</span>
              <span class="pm-rv">{{ fmtMoney(effective.price) }}</span></div>
            <div class="pm-rrow pm-rrow-hl"><span class="pm-rk">Final price</span>
              <span class="pm-rv clr-green">{{ fmtMoney(effective.finalPrice) }}</span></div>
            <div class="pm-rrow"><span class="pm-rk">Discount</span>
              <span class="pm-rv clr-red">-{{ fmtMoney((effective.price ?? 0) - (effective.finalPrice ?? 0)) }}</span></div>
            <div class="pm-rrow"><span class="pm-rk">Promotion</span>
              <span class="pm-rv">
                <span v-if="effective.promotionCode" class="pm-tag pm-tag-y">{{ effective.promotionCode }}</span>
                <span v-else class="pm-dim">none</span>
              </span>
            </div>
            <div v-if="effective.comboInfo" class="pm-rrow">
              <span class="pm-rk">Combo</span>
              <span class="pm-rv"><span class="pm-tag pm-tag-g">{{ effective.comboInfo }}</span></span>
            </div>
          </div>
          ...
          <div class="pm-inline">
            <input v-model.number="customerIdForPrice" class="pm-in"
              placeholder="Customer ID" style="width:130px" type="number" />
            <button class="pm-btn pm-btn-outline" :class="{ loading: effCustLoading }"
              @click="loadEffectiveForCustomer" :disabled="effCustLoading">
              <span v-if="!effCustLoading">Tải</span><span v-else class="pm-spin"></span>
            </button>
          </div>
          <div v-if="effectiveForCustomer" class="pm-result mt-2">
            <div class="pm-rrow"><span class="pm-rk">Variant</span>
              <span class="pm-rv">{{ effectiveForCustomer.variantName ?? "—" }}</span></div>
            <div class="pm-rrow"><span class="pm-rk">Base price</span>
              <span class="pm-rv">{{ fmtMoney(effectiveForCustomer.price) }}</span></div>
            <div class="pm-rrow pm-rrow-hl"><span class="pm-rk">Final price</span>
              <span class="pm-rv clr-green">{{ fmtMoney(effectiveForCustomer.finalPrice) }}</span></div>
            <div class="pm-rrow"><span class="pm-rk">Promotion</span>
              <span class="pm-rv">
                <span v-if="effectiveForCustomer.promotionCode" class="pm-tag pm-tag-y">
    ...
    <div v-show="activeTab === 'history'" class="pm-pane">
      <div class="pm-card mb3">
        ...
        <div class="pm-inline">
          <input v-model.number="productId" class="pm-in" placeholder="Product ID"
            style="width:140px" type="number" />
          <button class="pm-btn pm-btn-primary" :class="{ loading: prodLoading }"
            @click="loadByProduct" :disabled="prodLoading">
        ...
        <div v-if="productPrices.length" class="pm-tw mt3">
          <table class="pm-tbl">
            <thead>
              <tr><th>Variant ID</th><th>Tên variant</th><th>SKU</th>
                <th>Base price</th><th>Final price</th><th>Promotion</th></tr>
            </thead>
            <tbody>
              <tr v-for="row in productPrices" :key="row.variantId">
                ...
                <td :class="row.finalPrice < row.price ? 'clr-green bold' : ''">
                  {{ fmtMoney(row.finalPrice) }}</td>
                <td>
                  <span v-if="row.promotionCode" class="pm-tag pm-tag-y">{{ row.promotionCode }}</span>
      ...
      <div class="pm-card">
        ...
        <div class="pm-inline">
          <input v-model.number="histVariantId" class="pm-in" placeholder="Variant ID"
            style="width:140px" type="number" />
          <button class="pm-btn pm-btn-primary" :class="{ loading: histLoading }"
            @click="loadVariantHistory" :disabled="histLoading">
        ...
        <div v-if="variantHistory.length" class="pm-tw mt3">
          <table class="pm-tbl">
            <thead>
              <tr><th>ID</th><th>Giá bán</th><th>Giá nhập</th><th>Lý do</th>
                <th>Hiệu lực từ</th><th>Hiệu lực đến</th><th>Người tạo</th><th>Action</th></tr>
            </thead>
            <tbody>
              <tr v-for="row in variantHistory" :key="row.id"
                ...
                <td>
                  <div class="pm-acts">
                    ...
                    <button v-if="!row.effectiveTo" class="pm-xs pm-xs-o"
                      @click="openEdit(row)">Sửa</button>
                    <button class="pm-xs pm-xs-d" @click="removeHistory(row)">Xóa</button>
                  </div>
                </td>
              </tr>
            </tbody>
          ...
          <!-- Phân trang -->
          <div class="pm-pagination mt3">
            <button class="pm-btn pm-btn-ghost" :disabled="histPage === 0"
              @click="changeHistPage(histPage - 1)">← Trước</button>
            <span class="pm-dim mx2">Trang {{ histPage + 1 }} / {{ histTotalPages }}</span>
            <button class="pm-btn pm-btn-ghost"
              :disabled="histPage >= histTotalPages - 1"
              @click="changeHistPage(histPage + 1)">Sau →</button>
          </div>
        </div>
    ...
    <div v-show="activeTab === 'warning'" class="pm-pane">
      <div class="pm-card mb3">
        <div class="pm-card-head">
          <span class="pm-card-title">Quét toàn hệ thống — Giá bán &lt; Giá nhập</span>
          <code class="pm-badge pm-badge-r">GET /api/prices/conflicts</code>
          <button class="pm-btn pm-btn-orange ml-auto"
            :class="{ loading: conflictsLoading }" @click="loadConflicts"
        ...
        <template v-if="conflictsLoaded">
          ...
          <template v-else>
            <div class="pm-alert pm-alert-r mt3">
              ⚠️ {{ priceConflicts.length }} variant có giá bán thấp hơn giá nhập
            </div>
            <div class="pm-tw mt3">
              <table class="pm-tbl">
                <thead>
                  <tr><th>Variant ID</th><th>Tên variant</th><th>SKU</th>
                    <th>Giá bán</th><th>Giá nhập</th><th>Chênh lệch</th>
                    <th>Currency</th><th>Action</th></tr>
                </thead>
                <tbody>
      ...
      <div class="pm-card">
        ...
        <div class="pm-inline">
          <input v-model.number="costWarnVariantId" class="pm-in"
            placeholder="Variant ID" style="width:140px" type="number" />
          <button class="pm-btn pm-btn-orange" :class="{ loading: costWarnLoading }"
            @click="loadCostWarning" :disabled="costWarnLoading">
        ...
        <div v-if="costWarning" class="mt3">
          ...
          <div class="pm-result mt2">
            <div class="pm-rrow"><span class="pm-rk">Variant ID</span>
              <span class="pm-rv mono">{{ costWarning.variantId }}</span></div>
            <div class="pm-rrow"><span class="pm-rk">Tên</span>
              <span class="pm-rv">{{ costWarning.variantName ?? "—" }}</span></div>
            <div class="pm-rrow"><span class="pm-rk">Giá bán</span>
              <span class="pm-rv" :class="costWarning.belowCost ? 'clr-red' : ''">
                {{ fmtMoney(costWarning.price) }}</span></div>
            <div class="pm-rrow"><span class="pm-rk">Giá nhập</span>
              <span class="pm-rv">
                {{ costWarning.costPrice ? fmtMoney(costWarning.costPrice) : "—" }}</span></div>
          </div>
          <button v-if="costWarning.belowCost" class="pm-btn pm-btn-orange mt2"
            @click="openQuickFix({ variantId: costWarning.variantId, variantName: costWarning.variantName, price: costWarning.price, costPrice: costWarning.costPrice })">
            ✏️ Sửa giá ngay
          </button>
        </div>
      </div>
      ...
      <div class="pm-grid2">
        <div class="pm-card">
          ...
          <div class="pm-field">
            ...
            <input v-model.number="calcForm.customerId" class="pm-in"
              placeholder="VD: 1 — Nguyễn Minh Tuấn VIP Gold" type="number" />
          </div>
          <div class="pm-field mt3">
            ...
            <div class="pm-items">
              <div v-for="(item, idx) in calcForm.items" :key="idx" class="pm-item-row">
                ...
                <input v-model.number="item.variantId" class="pm-in"
                  placeholder="Variant ID" style="width:120px" type="number" />
                <span class="pm-item-sep">×</span>
                <input v-model.number="item.quantity" class="pm-in"
                  placeholder="SL" style="width:60px" type="number" :min="1" />
                <button class="pm-icon-btn" @click="calcForm.items.splice(idx, 1)">✕</button>
              </div>
            </div>
            <button class="pm-btn pm-btn-ghost mt2"
              @click="calcForm.items.push({ variantId: null, quantity: 1 })">
              + Thêm sản phẩm
            </button>
          </div>
          <div class="pm-foot">
            <button class="pm-btn pm-btn-primary" :class="{ loading: calcLoading }"
              @click="calculateOrder" :disabled="calcLoading">
        ...
        <div class="pm-card">
          ...
          <div v-if="calcResult">
            <div class="pm-calc-box">
              <div class="pm-calc-r">
                <span>Tổng giá gốc</span><span>{{ fmtMoney(calcResult.subtotal) }}</span></div>
              <div class="pm-calc-r pm-calc-r-disc">
                <span>Giảm giá</span><span>-{{ fmtMoney(calcResult.discountTotal) }}</span></div>
              <div v-if="calcResult.freeQty" class="pm-calc-r">
                <span>Tặng kèm</span>
                <span class="pm-tag pm-tag-g">{{ calcResult.freeQty }} sp miễn phí</span></div>
              <div class="pm-calc-sep"></div>
              <div class="pm-calc-r pm-calc-r-total">
                <span>Tổng thanh toán</span>
                <span class="clr-green">{{ fmtMoney(calcResult.finalTotal) }}</span></div>
            </div>
            <div v-if="calcResult.items?.length" class="pm-tw mt3">
              <table class="pm-tbl pm-tbl-sm">
                <thead>
                  <tr><th>Variant</th><th>Tên</th><th>SL</th>
                    <th>Đơn giá</th><th>Sau KM</th><th>Thành tiền</th></tr>
                </thead>
                <tbody>
                  <tr v-for="item in calcResult.items" :key="item.variantId">
                    ...
                    <td :class="item.finalUnitPrice < item.unitPrice ? 'clr-green bold' : ''">
                      {{ fmtMoney(item.finalUnitPrice) }}</td>
    ...
    <div v-show="activeTab === 'dashboard'" class="pm-pane">
      <div class="pm-inline mb3">
        ...
        <button class="pm-btn pm-btn-outline ml-auto" :class="{ loading: dashLoading }"
          @click="loadDashboard" :disabled="dashLoading">
      ...
      <template v-if="dashData">
        <div class="pm-stats4">
          ...
          <div class="pm-stat" :class="(dashData.variantsBelowCost ?? 0) > 0 ? 'pm-stat-r' : 'pm-stat-dim'">
            <div class="pm-stat-ico">⚠️</div>
            <div class="pm-stat-n">{{ dashData.variantsBelowCost ?? 0 }}</div>
            <div class="pm-stat-l">Giá dưới giá nhập</div>
          </div>
          <div class="pm-stat" :class="(dashData.expiringIn3Days ?? 0) > 0 ? 'pm-stat-o' : 'pm-stat-dim'">
            <div class="pm-stat-ico">⏰</div>
            <div class="pm-stat-n">{{ dashData.expiringIn3Days ?? 0 }}</div>
            <div class="pm-stat-l">Sắp hết hạn (3 ngày)</div>
          </div>
          <div class="pm-stat" :class="(dashData.conflictCount ?? 0) > 0 ? 'pm-stat-p' : 'pm-stat-dim'">
            <div class="pm-stat-ico">🔀</div>
        ...
        <div class="pm-card mt3">
          <div class="pm-card-head">
            ...
            <span v-if="dashData.promotionConflicts?.length"
              class="pm-tag pm-tag-r">{{ dashData.promotionConflicts.length }}</span>
          </div>
          <div v-if="dashData.promotionConflicts?.length" class="pm-tw mt2">
            <table class="pm-tbl">
              <thead>
                <tr><th>ID</th><th>Code</th><th>Tên chiến dịch</th><th>Lý do xung đột</th></tr>
              </thead>
              <tbody>
    ...
    <div v-if="dlg.open" class="pm-overlay" @click.self="dlg.open = false">
      <div class="pm-dialog">
        ...
        <div v-if="dlg.isQuickFix" class="pm-alert pm-alert-r mb3">
          ⚠️ Giá bán hiện tại <strong>{{ fmtMoney(dlg.currentPrice) }}</strong>
          thấp hơn giá nhập <strong>{{ fmtMoney(dlg.currentCostPrice) }}</strong>
        </div>
        ...
        <div class="pm-row2 mb3">
          ...
          <div class="pm-field">
            ...
            <span v-if="dlg.form.price && dlg.form.costPrice && dlg.form.price < dlg.form.costPrice"
              class="pm-in-hint clr-red">⚠️ Vẫn thấp hơn giá nhập!</span>
          </div>
        </div>
        <div class="pm-row2">
          <div class="pm-field">
        ...
        <div class="pm-dlg-foot">
          ...
          <button class="pm-btn pm-btn-primary" :class="{ loading: dlg.loading }"
...
const setForm = reactive({
  variantId: null, currencyCode: "VND", price: 0, costPrice: 0, reason: ""
});
...
async function loadEffectiveForCustomer() {
  ...
  try {
    const r = await pricesApi.getEffectiveForCustomer(setForm.variantId, customerIdForPrice.value);
...
// ─── EDIT DIALOG ───
const dlg = reactive({
  open: false, loading: false, alert: "",
  id: null, variantId: null, isQuickFix: false,
  currentPrice: null, currentCostPrice: null,
...
async function calculateOrder() {
  ...
  try {
    const r = await pricesApi.calculateOrder(calcForm.customerId || null, items);
